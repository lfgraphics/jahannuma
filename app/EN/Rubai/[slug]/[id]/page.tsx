...
export default function Page() {
  ...
  const shaerName = useMemo(() => data?.enShaer ?? data?.shaer ?? "", [data]);
  const unwan = useMemo(() => data?.enUnwan ?? data?.unwan ?? "", [data]);
  const bodyLines = useMemo(() => {
    const raw = data?.enBody ?? data?.body ?? "";
  ...
  const visitRubai = () => {
    if (typeof window !== undefined) {
      ...
      if (!referrer.includes("/Rubai")) {
        window.location.href = `${window.location.origin}/EN/Rubai`;
      } else {
        window.history.back();
      }
    }
  ...
  return (
    <div dir="ltr" className="flex justify-center">
      {(listLoading && recordLoading) ? (
        ...
        <div className="p-4 mt-3 w-screen md:w-[400px] text-center">Record is not available.</div>
      ) : (
        <div className="p-4 mt-3 w-full md:w-[400px]">
          ...
          <div className="ghazalHead mb-3 text-primary">
            <Link href={`/EN/Shaer/${encodeURIComponent(String(shaerName).replace(/\s+/g, "-"))}?tab=intro`}>
              <h2>{shaerName}</h2>
            </Link>
          </div>
          ...
          <div className="mazeed flex justify-around" data-aos="fade-up">
            ...
            <Link
              href={`/EN/Rubai/shaer/${encodeURIComponent(String(shaerName).replace(/\s+/g, "-"))}`}
              className="text-blue-600 underline"
              scroll={false}
            >
              {shaerName ? `More rubai by ${shaerName}` : "More Rubai"}
            </Link>
          </div>
        </div>
      )}
