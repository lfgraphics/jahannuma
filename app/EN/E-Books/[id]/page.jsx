...
const App = ({ params }) => {
  ...
  return (
    <div dir="ltr" className="flex flex-col min-h-screen w-screen bg-white">
            ...
            <div className="details" data-aos="fade-up">
              {data && (
                <div className="text-lg" data-aos="fade-up">
                  <p className="my-2">Kitab ka nam: {data.enBookName}</p>
                  <p className="my-2">
                    Isha'at: {formatDate(data.publishingDate)}
                  </p>
                  <p className="my-2">Musannif: {data.enWriter}</p>
                  <p className="my-2">Tafseel: {data.enDesc}</p>
                  {data.enMaloomat && (
                    <div className="bg-gray-100 py-2 mt-4 rounded-sm border">
                      <p className="text-xs mr-4 text-black">Dilchasp Ma'loomat:</p>
                  ...
                  <div className="scale-75">
                    <Link
                      href={`#pdf`}
                      className=" text-white block mx-auto m-4 p-2 bg-blue-500 text-center w-[210px] px-1 rounded-md"
                    >
                      Kitab parhen
                    </Link>
                    {bookUrl && data.download && (
                      <Link
                        href={bookUrl}
                        className=" text-black block mx-auto m-4 mb-1 p-2 border-2 border-blue-500 text-center w-[210px] px-1 rounded-md"
                      >
                        Kitab download karen
                      </Link>
                    )}
                  </div>
