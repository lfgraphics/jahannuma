    ...
    try {
      ...
      const formattedRecords: EBooksType[] = result.records.map(
        (record: any) => ({
          ...
          fields: {
            ...record.fields,
            bookName: record.fields?.enBookName,
            writer: record.fields?.enWriter,
            publishingData: record.fields?.publishingData,
            tafseel: record.fields?.enDsc,
    ...
    if (typeof window !== undefined && window.localStorage) {
      try {
        ...
        if (!isDuplicate) {
          ...
          showToast(
            "success",
            "Aap ki profile me yeh kitab kamiyabi ke sath jod di gai hai"
          );
          ...
          showToast(
            "invalid",
            "Aap ki profile se yeh kitab kamiyabi ke sath hata di gai hai"
          );
            ...
            <div className="flex justify-center text-lg m-5">
              ...
              >
                {moreloading
                  ? "Loading ..."
                  : noMoreData
                  ? "Mazeed kitaben maujood nahin hain"
                  : "Mazeed kitaben load karen"}
              </button>
            </div>
          )}
        </div>
