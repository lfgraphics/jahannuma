...
const Page: React.FC<PageProps> = ({ initialData = [] }) => {
  ...
  // Build filter formula - prioritize English fields first, then fallback
  const filterFormula = useMemo(() => {
    ...
    // Priority order: English fields first, then other languages
    return `OR( FIND('${escaped}', LOWER({enWriter})), FIND('${escaped}', LOWER({writer})), FIND('${escaped}', LOWER({hiWriter})), FIND('${escaped}', LOWER({enBookName})), FIND('${escaped}', LOWER({bookName})), FIND('${escaped}', LOWER({hiBookName})), FIND('${escaped}', LOWER({enDesc})), FIND('${escaped}', LOWER({desc})), FIND('${escaped}', LOWER({hiDesc})), FIND('${escaped}', LOWER({publishingDate})) )`;
  }, [debouncedSearchText]);
  ...
  const formattedRecords: EBooksType[] = useMemo(() => {
    ...
    const formatted = (dataToUse || []).map((record: any) => ({
      ...
      fields: {
        ...record.fields,
        // Use English fields with fallback for display
        bookName: getLanguageFieldValue(record.fields, 'bookName', 'EN', ['EN', 'UR', 'HI']) || record.fields?.bookName,
        writer: getLanguageFieldValue(record.fields, 'writer', 'EN', ['EN', 'UR', 'HI']) || record.fields?.writer,
        publishingData: record.fields?.publishingData,
        tafseel: getLanguageFieldValue(record.fields, 'desc', 'EN', ['EN', 'UR', 'HI']) || record.fields?.desc,
    ...
    if (debouncedSearchText.trim()) {
      ...
      return formatted.sort((a, b) => {
        // Get English fields with fallback
        const aWriter = (getLanguageFieldValue(a.fields, 'writer', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
        const aBookName = (getLanguageFieldValue(a.fields, 'bookName', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
        const aDesc = (getLanguageFieldValue(a.fields, 'desc', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
        const aOther = `${a.fields.publishingDate || ""}`.toLowerCase();

        const bWriter = (getLanguageFieldValue(b.fields, 'writer', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
        const bBookName = (getLanguageFieldValue(b.fields, 'bookName', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
        const bDesc = (getLanguageFieldValue(b.fields, 'desc', 'EN', ['EN', 'UR', 'HI']) || "").toLowerCase();
