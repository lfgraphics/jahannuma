      ...
      {insideBroser && mutalaData !== undefined && (
        <>
          <h1 className="text-4xl text-center mt-7 mb-3 text-[#984A02]">
            {mutalaData?.fields?.enUnwan || "Study Results"}
          </h1>
          <h2 className="text-2xl text-center mb-2">
            {mutalaData?.fields?.enHeading || mutalaData?.fields?.heading}
          </h2>
          <p dir="ltr" className="p-6 bg-[#F3F4F6] dark:bg-[#2d2d2f] leading-10">
            {mutalaData?.fields?.enBody || mutalaData?.fields?.body}
          </p>
          <div
            dir="ltr"
          ...
          >
            {(mutalaData.fields.enHawalaLink || mutalaData.fields.hawalaLink) ? (
              <p>
                ...
                >
                  {mutalaData?.fields?.enHawalaText || mutalaData?.fields?.hawalaText}
                </Link>
              </p>
            ) : (
              `Reference: ${mutalaData?.fields?.enHawalaText || mutalaData?.fields?.hawalaText}`
            )}
            {/* writer */}
            <p className="writer">{mutalaData.fields.enWriter || mutalaData.fields.writer}</p>
          </div>
        </>
      )}
    </div>
