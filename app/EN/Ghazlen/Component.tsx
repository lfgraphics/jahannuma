    ...
    try {
      ...
      if (userQuery) {
        ...
        const encodedFormula = encodeURIComponent(
          ...
          FIND('${searchText.trim().toLowerCase()}', LOWER({enShaer})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({enGhazalHead})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({enGhazal})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({enUnwan})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({hiShaer})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({hiGhazalHead})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({hiGhazal})),
          FIND('${searchText.trim().toLowerCase()}', LOWER({hiUnwan}))
        )`
        );
      ...
      const formattedRecords = records.map((record: any) => ({
        ...
        fields: {
          ...record.fields,
          shaer: record.fields?.enShaer?.split("\n"),
          ghazal: record.fields?.enGhazal?.split("\n"),
          ghazalHead: record.fields?.enGhazalHead?.split("\n"),
          unwan: record.fields?.enUnwan?.split("\n"),
        },
      }));
      if (!offset) {
        if (userQuery) {
  ...
  const handleShareClick = async (
    ...
    try {
      if (navigator.share) {
        navigator
          .share({
            title: shaerData.fields.enShaer, // Use the shaer's name as the title
            text:
        ...
        <div className="w-screen h-screen bg-black bg-opacity-60 flex flex-col justify-center fixed z-50">
          ...
          >
            <div className="dialog-content">
              <p className="text-lg pb-3 border-b">
                Barahekaram apna nam darj karen
              </p>
              <p className="pt-2">
                {" "}Aap ka nam sirf aap ke tabseron ko aap ke nam se dikhane ke liye kiya jaega
              </p>
              <input
                type="text"
                id="nameInput"
                placeholder="Aap ka nam"
        ...
        <div className="filter-btn basis-[75%] justify-center text-center flex">
          ...
          >
            ...
            <input
              type="text"
              dir="ltr"
              placeholder="Likh kar talash karen"
              ...
              <div className="flex justify-center text-lg m-5">
                ...
                >
                  {moreloading
                    ? "Loading..."
                    : noMoreData
                    ? "Mazeed ghazlen nahin hain"
                    : "Mazeed Ghazlen Load karen"}
                </button>
              </div>
            )}
          </div>
