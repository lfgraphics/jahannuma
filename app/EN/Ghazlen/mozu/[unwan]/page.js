...
const Page = ({ params }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const url = `https://api.airtable.com/v0/${BASE_ID}/${TABLE_NAME}?filterByFormula=FIND('${decodedUnwan}', ARRAYJOIN({enUnwan}, ' '))`;
    ...
    if (typeof window !== undefined && window.localStorage && e.detail == 1) {
      try {
        ...
        if (!isDuplicate) {
          ...
          showToast(
            "success",
            "Aapki profile me ye ghazal jod di gai hai "
          );
          try {
          ...
          showToast(
            "invalid",
            "Aapki profile se yah ghazal kamiyabi ke sath hata dee gai hai"
          );
          try {
            // Make API request to update the record's "Likes" field
            const updatedLikes = shaerData.fields.likes - 1;
      ...
      if (navigator.share) {
        navigator
          .share({
            ...
            url: `${window.location.href.split("/EN/Ghazlen/")[0] + "/EN/Ghazlen/" + shaerData.id}`, // Get the current page's URL
          })
  ...
  const handleCardClick = (shaerData) => {
    ...
    setSelectedCard({
      id: shaerData.id,
      fields: {
        shaer: shaerData.fields.enShaer,
  ...
  return (
    <div>
      ...
      <div className="flex flex-row w-screen bg-white border-b-2 p-3 justify-center items-center">
        <div className="text-xl m-5">{`Ghazlen Ba'unwan : ${decodedUnwan}`}</div>
      </div>
