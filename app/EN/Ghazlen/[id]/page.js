...
const Page = ({ params }) => {
  ...
  const ghazalLines = data.enGhazal?.split('\n');
  const anaween = data.enUnwan?.split('\n');

  const visitGhazlen = () => {
    if (typeof window !== undefined) {
      const referrer = document.referrer || '';
      // Check if the referrer is not coming from /Ghazlen
      if (!referrer.includes('/EN/Ghazlen')) {
  ...
  const downloadImageWithWatermark = async () => {
    ...
    try {
      ...
      link.href = dataUrl;
      link.download = `${data.enGhazalHead || "ghazal"}.png`;
      link.click();
      ...
      <div className="w-full sm:w-[400px]">
        {loading ? <ComponentsLoader /> : (
          <div id="main" className="p-4 mt-3">
            <div className={`ghazalHead text-lg text-black text-center`}>
              {data.enGhazalHead?.split('\n').map((line, index) => (
                <h2
                  key={index}
            ...
            <div className="shaer my-3 text-[#984A02]">
              <Link href={`/EN/Shaer/${data.shaer}`}>
                <h2>{data.enShaer}</h2>
              </Link>
            </div>
            ...
            <div className="text-2xl mb-4 flex flex-col justify-center">
              {ghazalLines?.map((line, index) => (
                <p
                  data-aos="fade-up"
                  key={index}
                  className="justif w-full px-6 text-black pb-3 text-2xl [&]:text-[length:clamp(1rem,2vw,1.5rem)] break-words"
                >
                  {line}
                </p>
              ))}
            ...
            <div className="flex gap-5 text-md mb-4 justify-center" data-aos="fade-up">
              {anaween?.map((unwan, index) => (
                <Link href={`/EN/Ghazlen/mozu/${unwan}`} className={`unwan text-blue-500 underline cursor-pointer`} style={{ lineHeight: "normal" }} key={index}>{unwan}</Link>
              ))}
            </div>
          </div>
        )}
        {!loading && (
          <div className="mazeed flex flex-col px-4 gap-4 mb-4 justify-around" data-aos="fade-up">
            <button
              onClick={visitGhazlen}
              className="bg-white text-[#984A02] border active:bg-[#984a02ac] active:text-white border-[#984A02] px-4 py-2 pt-3 rounded-md"
            >
              Mazeed Ghazlen
            </button>
            <Link scroll={false} href={`/EN/Ghazlen/shaer/${data?.shaer?.replace(' ', '_')}`} className="text-blue-600 underline">{data.enShaer} ki Mazeed ghazlen</Link>
            <button
              onClick={downloadImageWithWatermark}
              className="bg-green-500 text-white px-4 py-2 pt-3 rounded-md hover:bg-green-600 transition"
            >
              Tasweer ki shakal men download karen
            </button>
          </div>
        )}
      </div>
