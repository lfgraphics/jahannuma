...
import { createRecord, listRubaiRecords } from "@/lib/airtable/airtable-client";
import { formatRubaiRecord } from "@/lib/airtable/airtable-utils";
import { RubaiListResponse } from "@/types/api/responses";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response: RubaiListResponse = {
      success: true,
      data: {
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code: "CREATE_FAILED",
          message: "Failed to create rubai",
          details: error instanceof Error ? error.message : "Unknown error",
        },
      },
      { status: 500 }
