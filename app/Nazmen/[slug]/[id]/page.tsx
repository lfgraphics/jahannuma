...
import type { AirtableRecord, NazmenRecord, AshaarPageParams } from "@/app/types";
...
export default function Page() {
    ...
    const { records, isLoading: listLoading, error: listError } = useAirtableList<AirtableRecord<NazmenRecord>>(BASE_ID, TABLE, {
    ...
    const shouldTryRecord = !!id && !forceList && (isRecordId || (enableList && !listLoading && (!records || records.length === 0)));
    ...
    return (
        <div dir="rtl" className="flex justify-center">
            {(!id) ? (
                <div className="p-4 mt-3 w-screen md:w-[400px] text-center">Invalid record id</div>
                ...
                <div className="p-4 mt-3 w-screen md:w-[400px]">
                    ...
                    {!fields?.paband && (
                        <div className="text-2xl mb-4 text-center">
                            {ghazalLines?.map((line: string, index: number) => (
                                <p className={`${line == "****" ? "my-12 md:my-9 opacity-0" : ""}`} style={{ lineHeight: "normal" }} key={index}>
                                    {line}
                                </p>
                            ))}
                        </div>
                    ...
                    <div className="mazeed flex justify-around">
                        <button onClick={visitNazmen} className="bg-white text-[#984A02] border active:bg-[#984A02] active:text-white border-[#984A02] px-4 py-2 rounded-md">
                            مزید نظمیں
                        </button>
                        <Link href={`/Nazmen/shaer/${encodeURIComponent((fields?.shaer ?? "").replace(/\s+/g, "_"))}`} className="text-blue-600 underline">
