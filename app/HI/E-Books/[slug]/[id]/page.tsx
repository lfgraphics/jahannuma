...
export default function Page() {
  ...
  const handleShareClick = () => {
    try {
      if (navigator.share) {
        const title = record?.fields?.bookName || "पुस्तक";
      ...
      {!isLoading && (
        <div className="relative">
            ...
            <div className="details" data-aos="fade-up">
              {record?.fields ? (
                <div className="text-lg" data-aos="fade-up">
                  ...
                  <p className="my-2">
                    लेखक:
                    <Link href={`/HI/Shaer/${record.fields.writer}`}>
                      {record.fields.writer}
                    </Link>
                  </p>
                  <p className="my-2">विवरण: {record.fields.desc}</p>
                  {record.fields.maloomat && (
                    <div className="bg-muted py-2 mt-4 rounded-sm border">
                      <p className="text-xs ml-4 text-foreground">
                        दिलचस्प जानकारी:
                      </p>
                      <p className="maloomat ml-4 mt-2 text-sm">
                        {record.fields.maloomat}
                      </p>
                    </div>
                  )}
          ...
          <Button onClick={() => setBookKey(Math.random() + 1)}>
            <RefreshCwIcon
              className="sticky top-[95px] left-3"
            />
            रीफ्रेश
          </Button>
