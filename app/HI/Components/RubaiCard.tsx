    ...
    <>
      ...
      >
        <div className="unwan text-center text-[#984A02] text-2xl mb-2">
          <p>{(RubaiData.fields as any).hiUnwan || RubaiData.fields.unwan}</p>
        </div>
        <div className="unwan flex flex-col w-[90%] justify-center mb-2">
          {((RubaiData.fields as any).hiBody || RubaiData?.fields?.body).split("\n").map((lin: any, index: number) => (
        ...
        <Link
          href={{ pathname: `/HI/Shaer/${encodeURIComponent(((RubaiData.fields as any).hiShaer || RubaiData.fields?.shaer || '').replace(/\s+/g, '-'))}`, query: { tab: "intro" } }}
          className="text-center"
        >
          <h2 className="text-foreground text-lg mb-4">{(RubaiData.fields as any).hiShaer || RubaiData.fields?.shaer}</h2>
        </Link>
