...
const Quiz = () => {
  ...
  const getOptionClasses = (index: number) => {
    ...
    if (optionNumber === selectedOption && optionNumber !== correctAnswer) {
      return `${baseClasses} bg-gradient-to-r from-red-500 to-red-400 text-white border-2 border-red-600 shadow-xl`;
    }
      ...
      {!loading && quizData && (
        <div className="max-w-4xl mx-auto">
          ...
          <h3 className="text-2xl md:text-3xl text-center mb-8 text-gray-800 font-medium">
            {quizData.fields?.hiQuestion}
          </h3>
          <div className="space-y-4">
            {["hiOpt1", "hiOpt2", "hiOpt3", "hiOpt4"].map((opt, index) => (
              <button
                key={opt}
