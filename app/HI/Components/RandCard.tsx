  ...
  const fetchData = async () => {
    try {
      ...
      const formattedRecords = records.map((record: any) => ({
        ...
        fields: {
          ...record.fields,
          ghazal: record.fields?.hiBody.split("\n"),
          shaer: record.fields?.hiShaer.split("\n"),
          ghazalHead: record.fields?.hiSher.split("\n"),
          unwan: record.fields?.hiUnwan.split("\n"),
        },
      }));
      ...
      console.log('randome one',randomRecord)

      setDataItems(randomRecord);
      console.log("record", formattedRecords)
      ...
      if (navigator.share) {
        ...
        try {
          ...
          const updateResponse = await fetch(
            `https://api.airtable.com/v0/appvzkf6nX376pZy6/Ashaar`,
            {
              method: "PATCH",
