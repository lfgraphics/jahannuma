...
export async function GET(request: NextRequest) {
  try {
    ...
    const sort = searchParams.get("sort") || undefined; // No default sort
    ...
    const fieldsToUse = fields || getMultilingualFieldsString('ghazlen');
...
export async function POST(req: NextRequest) {
  try {
    ...
    const record = await createRecord("Ghazlen", fields);
    return ok({ record });
  } catch (error) {
    return errors.internal(
      "Failed to create ghazal",
      error instanceof Error ? error.message : "Unknown error"
    );
  }
}
