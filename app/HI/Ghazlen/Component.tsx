...
const GhazlenComponent: React.FC<GhazlenProps> = ({ initialData = [], language }) => {
  ...
  // Build filter formula via SWR hook - prioritize shaer (poet name) with Hindi fields
  const filterFormula = useMemo(() => {
    ...
    return `OR( FIND('${safeQ}', LOWER({hiShaer})), FIND('${safeQ}', LOWER({shaer})), FIND('${safeQ}', LOWER({hiGhazalHead})), FIND('${safeQ}', LOWER({ghazalHead})), FIND('${safeQ}', LOWER({hiGhazal})), FIND('${safeQ}', LOWER({ghazal})), FIND('${safeQ}', LOWER({hiUnwan})), FIND('${safeQ}', LOWER({unwan})) )`;
  }, [debouncedSearchText]);
  ...
  const formattedRecords: Shaer[] = useMemo(() => {
    const formatted = (records || []).map((record: any) => {
      ...
      const shaerName = getEnhancedLanguageFieldValue(record.fields, 'shaer', 'HI', 'ghazlen') || record.fields.shaer;
      const ghazalHead = getEnhancedLanguageFieldValue(record.fields, 'ghazalHead', 'HI', 'ghazlen') || record.fields.ghazalHead;
      const ghazal = getEnhancedLanguageFieldValue(record.fields, 'ghazal', 'HI', 'ghazlen') || record.fields.ghazal;
      const unwan = getEnhancedLanguageFieldValue(record.fields, 'unwan', 'HI', 'ghazlen') || record.fields.unwan;
      const description = getEnhancedLanguageFieldValue(record.fields, 'description', 'HI', 'ghazlen') || record.fields.hiDescription;
      const takhallus = getEnhancedLanguageFieldValue(record.fields, 'takhallus', 'HI', 'ghazlen') || record.fields.hiTakhallus;
              ...
              <div className="flex justify-center text-lg m-5">
                ...
                >
                  {isLoadingMore
                    ...
                    : noMoreData
                      ? "कोई और ग़ज़लें उपलब्ध नहीं हैं"
                      : uiTexts.buttons.loadMore.HI + " ग़ज़लें"}
                </button>
              </div>
            )}
          </div>
