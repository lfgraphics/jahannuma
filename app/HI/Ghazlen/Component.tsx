...
const Ghazlen: React.FC<GhazlenProps> = ({ initialData = [] }) => {
  ...
  // Build filter formula via SWR hook - prioritize shaer (poet name) - using Hindi fields
  const filterFormula = useMemo(() => {
    ...
    // Priority order: hiShaer (poet name), then hiGhazalHead, hiGhazal, hiUnwan, with fallbacks to default fields
  ...
  const formattedRecords: Shaer[] = useMemo(() => {
    const formatted = (records || []).map((record: any) => ({
      ...
      fields: {
        ...
        ghazal: record.fields?.hiGhazal
          ? String(record.fields.hiGhazal).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : String(record.fields?.ghazal || "").replace(/\r\n?/g, "\n").split("\n").filter(Boolean),
        ghazalHead: record.fields?.hiGhazalHead
          ? String(record.fields.hiGhazalHead).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : record.fields?.ghazalHead || [],
        unwan: record.fields?.hiUnwan
          ? String(record.fields.hiUnwan).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : record.fields?.unwan || [],
        // Keep Hindi fields for reference
        hiShaer: record.fields?.hiShaer,
        hiGhazal: record.fields?.hiGhazal
          ? String(record.fields.hiGhazal).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : undefined,
        hiUnwan: record.fields?.hiUnwan
          ? String(record.fields.hiUnwan).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : undefined,
      },
    }));
