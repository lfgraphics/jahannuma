...
const Nazmen: React.FC<NazmenProps> = ({ initialData }) => {
  ...
  // List records with SWR and filter - prioritize shaer (poet name) - using Hindi fields
  const filterFormula = useMemo(() => {
    const q = debouncedSearchText.trim().toLowerCase();
    if (!q) return undefined;
    const escapedQ = escapeAirtableFormulaValue(q);
    // Priority order: hiShaer (poet name), then hiNazm, hiUnwan, with fallbacks to default fields
    return `OR( FIND('${escapedQ}', LOWER({hiShaer})), FIND('${escapedQ}', LOWER({shaer})), FIND('${escapedQ}', LOWER({displayLine})), FIND('${escapedQ}', LOWER({hiNazm})), FIND('${escapedQ}', LOWER({nazm})), FIND('${escapedQ}', LOWER({hiUnwan})), FIND('${escapedQ}', LOWER({unwan})) )`;
  }, [debouncedSearchText]);
  ...
  const formattedRecords: Shaer[] = useMemo(() => {
    const formatted = (records || []).map((record: any) => ({
      ...
      fields: {
        ...
        ghazal: record.fields?.hiNazm
          ? String(record.fields.hiNazm).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
          : String(record.fields?.nazm || "").replace(/\r\n?/g, "\n").split("\n").filter(Boolean),
        ghazalHead: record.fields?.hiNazmHead
          ? String(record.fields.hiNazmHead).replace(/\r\n?/g, "\n").split("\n").filter(Boolean)
