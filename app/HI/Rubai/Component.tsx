...
const RubaiComponent: React.FC<RubaiComponentProps> = ({
  ...
  // Build filter formula and use new Rubai data hook - using Hindi fields
  const filterFormula = useMemo(() => {
    const q = debouncedSearchText.trim().toLowerCase();
    if (!q) return undefined;
    const escaped = q.replace(/'/g, "\\'");
    // Priority order: hiShaer (poet name), then hiBody, hiUnwan, with fallbacks to default fields
    return `OR( FIND('${escaped}', LOWER({hiShaer})), FIND('${escaped}', LOWER({shaer})), FIND('${escaped}', LOWER({hiBody})), FIND('${escaped}', LOWER({body})), FIND('${escaped}', LOWER({hiUnwan})), FIND('${escaped}', LOWER({unwan})) )`;
  }, [debouncedSearchText]);
  ...
  // Sort by search relevance when there's a search query - using Hindi fields
  const dataItems = useMemo(() => {
    ...
    if (debouncedSearchText.trim()) {
      ...
      sortedData = (records || []).slice().sort((a: Rubai, b: Rubai) => {
        ...
        const aShaer = ((a.fields as any).hiShaer || a.fields.shaer || "").toLowerCase();
        const aBody = ((a.fields as any).hiBody || a.fields.body || "").toLowerCase();
        const aUnwan = ((a.fields as any).hiUnwan || a.fields.unwan || "").toLowerCase();

        const bShaer = ((b.fields as any).hiShaer || b.fields.shaer || "").toLowerCase();
        const bBody = ((b.fields as any).hiBody || b.fields.body || "").toLowerCase();
        const bUnwan = ((b.fields as any).hiUnwan || b.fields.unwan || "").toLowerCase();
