...
const RubaiComponent: React.FC<RubaiComponentProps> = ({
  ...
  // Build filter formula and use new Rubai data hook with Hindi fields
  ...
  const dataItems = useMemo(() => {
    let sortedData = (records || []).map((record: Rubai) => {
      ...
      const shaerName = getEnhancedLanguageFieldValue(record.fields, 'shaer', 'HI', 'rubai') || record.fields.shaer;
      const body = getEnhancedLanguageFieldValue(record.fields, 'body', 'HI', 'rubai') || record.fields.body;
      const unwan = getEnhancedLanguageFieldValue(record.fields, 'unwan', 'HI', 'rubai') || record.fields.unwan;
      const description = getEnhancedLanguageFieldValue(record.fields, 'description', 'HI', 'rubai') || record.fields.hiDescription;
      const takhallus = getEnhancedLanguageFieldValue(record.fields, 'takhallus', 'HI', 'rubai') || record.fields.hiTakhallus;
              ...
              <div className="flex justify-center text-lg m-5">
                ...
                >
                  {isLoadingMore
                    ...
                    : !hasMore
                      ? "कोई और रुबाई उपलब्ध नहीं है"
                      : uiTexts.buttons.loadMore.HI + " रुबाई"}
                </button>
              </div>
            )}
          </div>
