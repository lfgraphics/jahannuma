...
import type { AirtableRecord, RubaiRecord } from "@/app/types";
...
export default function Page() {
  ...
  const { records, isLoading: listLoading, error: listError } = useAirtableList<AirtableRecord<RubaiRecord>>(
    BASE_ID,
    TABLE,
    {
  ...
  const { data: recordData, isLoading: recordLoading, error: recordError } = useAirtableRecord<AirtableRecord<RubaiRecord>>(BASE_ID, TABLE, recordIdForHook);
  ...
  const fields = formatted?.fields as RubaiRecord | undefined;

  const visitRubai = () => {
    if (typeof window !== "undefined") {
      const referrer = document.referrer || "";
      if (!referrer.includes("/HI/Rubai")) {
      ...
      {!effectiveId ? (
        ...
        <div className="p-4 mt-3 w-screen md:w-[400px]">
          <div className="ghazalHead text-2xl text-center leading-[3rem]">
            {ensureStringArray(fields?.rubaiHead).map((line: string, index: number) => (
          ...
          <div className="text-2xl mb-4 flex flex-col justify-center ">
            {(typeof fields?.body === 'string' ? fields.body.split('\n') : []).map((line: string, index: number) => (
          ...
          <div className="mazeed flex justify-around" data-aos="fade-up">
            <button onClick={visitRubai} className="bg-white text-[#984A02] border active:bg-[#984a02ac] active:text-white border-[#984A02] px-4 py-2 rounded-md">
              और रुबाई
            </button>
            <Link scroll={false} href={`/HI/Rubai/shaer/${fields?.shaer?.replace(" ", "_")}`} className="text-blue-600 underline">
              {fields?.shaer} की और रुबाई
            </Link>
          </div>
        </div>
      )}
