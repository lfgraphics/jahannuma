...
const Ashaar: React.FC<AshaarProps> = ({ initialData }) => {
  ...
  // Build filter formula used by SWR hook - prioritize shaer (poet name) - using Hindi fields
  const filterFormula = useMemo(() => {
    const q = debouncedSearchText.trim().toLowerCase();
    if (!q) return undefined;
    const safe = escapeAirtableFormulaValue(q);
    // Priority order: hiShaer (poet name), then hiSher, hiBody, hiUnwan, with fallbacks to default fields
    return `OR( FIND('${safe}', LOWER({hiShaer})), FIND('${safe}', LOWER({shaer})), FIND('${safe}', LOWER({hiSher})), FIND('${safe}', LOWER({sher})), FIND('${safe}', LOWER({hiBody})), FIND('${safe}', LOWER({body})), FIND('${safe}', LOWER({hiUnwan})), FIND('${safe}', LOWER({unwan})) )`;
  }, [debouncedSearchText]);
  ...
  // Format records and sort by search relevance - using Hindi fields
  const formattedRecords: Shaer[] = useMemo(() => {
    const formatted = (records || []).map((record: any) => ({
      ...record,
      fields: {
        ...record.fields,
        // Use Hindi fields with fallbacks to default fields
        shaer: record.fields?.hiShaer || record.fields?.shaer || "",
        sher: record.fields?.hiSher || record.fields?.sher || "",
        body: record.fields?.hiBody || record.fields?.body || "",
        ghazal: String(record.fields?.hiSher || record.fields?.sher || "").replace(/\r\n?/g, "\n").split("\n").filter(Boolean),
        ghazalHead: String(record.fields?.hiSher || record.fields?.sher || "").replace(/\r\n?/g, "\n").split("\n").filter(Boolean),
        unwan: String(record.fields?.hiUnwan || record.fields?.unwan || "").replace(/\r\n?/g, "\n").split("\n").filter(Boolean),
      },
    }));

    // Sort by search relevance when there's a search query - now using Hindi fields
    if (debouncedSearchText.trim()) {
      const query = debouncedSearchText.trim().toLowerCase();
      return formatted.sort((a, b) => {
        // Now using the Hindi fields that were set above
    ...
    await shareRecordWithCount(
      {
        section: "Ashaar",
        id: shaerData.id,
        title: (shaerData.fields as any).hiShaer || shaerData.fields.shaer,
        textLines: (shaerData.fields as any).hiGhazal || (shaerData.fields.ghazalHead ?? []),
