...
const AshaarComponent: React.FC<AshaarProps> = ({ initialData }) => {
  ...
  const filterFormula = useMemo(() => {
    ...
    // Search in both Hindi and default fields with language awareness
    return `OR( 
      FIND('${safe}', LOWER({shaer})), 
      FIND('${safe}', LOWER({hiShaer})), 
      FIND('${safe}', LOWER({sher})), 
      FIND('${safe}', LOWER({hiSher})), 
      FIND('${safe}', LOWER({body})), 
      FIND('${safe}', LOWER({hiBody})), 
      FIND('${safe}', LOWER({unwan})), 
      FIND('${safe}', LOWER({hiUnwan})) 
    )`;
  }, [debouncedSearchText]);
  ...
  const formattedRecords: Shaer[] = useMemo(() => {
    const formatted = (records || []).map((record: any) => {
      ...
      // Use enhanced language field transformation for Hindi with proper fallback
      const shaer = getEnhancedLanguageFieldValue(fields, 'shaer', 'HI', 'ashaar') || fields.shaer || '';
      const sher = getEnhancedLanguageFieldValue(fields, 'sher', 'HI', 'ashaar') || fields.sher || '';
      const body = getEnhancedLanguageFieldValue(fields, 'body', 'HI', 'ashaar') || fields.body || '';
      const unwan = getEnhancedLanguageFieldValue(fields, 'unwan', 'HI', 'ashaar') || fields.unwan || '';
      const description = getEnhancedLanguageFieldValue(fields, 'description', 'HI', 'ashaar') || fields.description || '';
      const takhallus = getEnhancedLanguageFieldValue(fields, 'takhallus', 'HI', 'ashaar') || fields.takhallus || '';
