...
export interface GhazlenListParams {
  pageSize?: number;
...
export interface UseGhazlenDataOptions {
...
const ghazlenFetcher = async (url: string) => {
...
export function useGhazlenData(
  params: GhazlenListParams = {},
  options: UseGhazlenDataOptions = {}
) {
  ...
  const getKey = (pageIndex: number, previousPageData: any): string | null => {
    ...
    return buildAirtableAPIURL('ghazlen', urlParams);
  };
