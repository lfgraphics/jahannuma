...
export interface AshaarListParams {
  pageSize?: number;
...
export interface UseAshaarDataOptions {
...
const ashaarFetcher = async (url: string) => {
  console.log('Ashaar fetcher - URL:', url);
  const response = await fetch(url);
  console.log('Ashaar fetcher - Response status:', response.status);
  if (!response.ok) {
    console.error('Ashaar fetcher - Error response:', response.status, response.statusText);
    throw new Error(`HTTP ${response.status}: ${response.statusText}`);
  }
  const data = await response.json();
  console.log('Ashaar fetcher - Data received:', data);
  return data;
};

export function useAshaarData(
  params: AshaarListParams = {},
  options: UseAshaarDataOptions = {}
) {
  ...
  const getKey = (pageIndex: number, previousPageData: any): string | null => {
    ...
    const url = buildAirtableAPIURL('ashaar', urlParams);
    console.log('Ashaar getKey - Generated URL:', url, 'for pageIndex:', pageIndex);
    return url;
  };
