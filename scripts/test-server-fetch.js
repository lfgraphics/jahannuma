...
async function testServerFetch() {
  ...
  try {
    ...
    console.log(
      ...
      process.env.NEXT_PUBLIC_Api_Token ? "Set" : "Not set"
    );

    console.log("\nTesting Ashaar data fetch...");
    // Import BASE_IDS from centralized constants
    const { BASE_IDS } = require("../src/lib/airtable/airtable-constants");

    const ashaarData = await fetchList(
      BASE_IDS.ASHAAR, // Use centralized base ID
      "Ashaar",
      { pageSize: 5 },
      {
        cache: false,
    ...
    console.log(
      "- Sample record:",
      ashaarData?.records?.[0] ? "Present" : "None"
    );
  } catch (error) {
