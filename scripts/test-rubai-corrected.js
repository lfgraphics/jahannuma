/**
 * Test Rubai fields using actual field names from Airtable screenshots
 */
...
async function testRubaiFields() {
  console.log("🔍 TESTING RUBAI FIELDS WITH ACTUAL NAMES");
  console.log("=========================================");
  ...
  // Test English fields individually (using actual field names from screenshot)
  console.log("\n🇬🇧 English Fields:");
  await testField(["enShaer"], "enShaer");
  await testField(["enBody"], "enBody");
  await testField(["enunwan"], "enunwan (lowercase)");

  // Test Hindi fields individually (using actual field names from screenshot)
  console.log("\n🇮🇳 Hindi Fields:");
  await testField(["hiShaer"], "hiShaer");
  await testField(["hiBody"], "hiBody");
  await testField(["hiunwan"], "hiunwan (lowercase)");
  ...
  // Test with English fields (correct case)
  ...
  await testField(
    ...
    "Base + all English (correct case)"
  );

  // Test the full multilingual set with correct field names
  console.log("\n🌐 Full Multilingual Set (Corrected):");
  const fullSet = [
    "shaer",
    "body",
    "unwan",
  ...
  await testField(fullSet, "Complete multilingual field set (corrected)");
}
